...
export function parseTargetPrefix(s, allowedPrefixes) {
	let i;
...
export class Isupport {
	...
	membershipModes() {
		let prefix = this.raw.get("PREFIX");
		return parseMembershipModes(prefix !== undefined ? prefix : STD_PREFIX);
	}

	chanTypes() {
		let chanTypes = this.raw.get("CHANTYPES");
...
export function forEachChannelModeUpdate(msg, isupport, callback) {
	...
	let membershipModes = isupport.membershipModes();
